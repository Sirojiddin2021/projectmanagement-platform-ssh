...
def home(Id):
    ...
    all_projects = dbsession.query(Projects.Name, Projects.Description, Projects.SD, Projects.DD, Sections.Name.label("Section_name"), Sections.Id.label("Section_ID"), Users.Username.label("Username")
                                   ).join(Sections, Sections.Id == Projects.SectionID).join(Users, Users.Id == session["user_id"]).where(Projects.UserID == session["user_id"]).all()
...
def index():
    ...
    query = dbsession.query(Sections).filter(or_(Sections.Name == "Tasks Due Soon") | (
        Sections.Name == "Favorites") | (Sections.Name == "Recent Projects")).all()
    if not query:
...
def sections(Id):
    """ Add sections """
    form = SectionAddForm()
    u_sections = dbsession.query(Sections.Id, Sections.Name, Sections.Description, Sections.RDT).join(
        Users, Users.Id == Sections.UserID).where(Sections.UserID == session["user_id"]).all()
    if form.validate_on_submit():
...
def delete_section(Id, sec_id):
    """ delete sections """
    section = dbsession.query(Sections).join(Users, Users.Id == Sections.UserID).filter(
        and_(Sections.Id == sec_id, Users.Id == session["user_id"])).first()
...
def projects(Id):
    ...
    # table user projects
    t_projects = dbsession.query(Projects.Id, Projects.Name, Projects.Description, Projects.SD, Projects.DD, Sections.Id.label("Section_id"), Sections.Name.label("Section_name")
                                 ).join(Sections, Sections.Id == Projects.SectionID).where(Projects.UserID == session["user_id"]).all()
    # user sections including default
    all_user_sections = dbsession.query(Sections).filter(
        or_(Sections.UserID == session["user_id"]) | (Sections.Name == 'Default')).all()

    # user custom sections only
    user_sections = dbsession.query(Sections.Id, Sections.Name, Sections.Description, Sections.RDT).join(
        Users, Users.Id == Sections.UserID).where(Sections.UserID == session["user_id"]).all()
    ...
    t_tasks = dbsession.query(Tasks.Id, Tasks.Name, Tasks.Description, Tasks.SD, Tasks.DD, Tasks.ProjectID, Tasks.PriorityID, Priorities.Name.label("Priority_name"), Projects.Name.label("Project_name")
                              ).join(Priorities, Priorities.Id == Tasks.PriorityID).join(Projects, Projects.Id == Tasks.ProjectID).where(Tasks.Recorder == session["user_id"]).all()

    # user subtasks
    t_subtasks = dbsession.query(Subtasks.Id, Subtasks.Name, Subtasks.Description, Subtasks.SD, Subtasks.DD, Subtasks.PriorityID, Priorities.Name.label("Priority_name"), Tasks.Id.label('Task_id'),
                                 Tasks.Name.label("Task_name"), Projects.Name.label("Project_name")).join(Priorities, Priorities.Id == Subtasks.PriorityID).join(Tasks, Tasks.Id == Subtasks.TaskID).join(Projects, Projects.Id == Tasks.ProjectID).where(Subtasks.Recorder == session["user_id"]).all()
...
def tasks(Id):
    """ Add tasks """
    form = TaskAddForm()
    u_tasks = dbsession.query(Tasks.Id, Tasks.Name, Tasks.Description, Tasks.SD, Tasks.DD, Tasks.ProjectID, Tasks.PriorityID, Priorities.Name.label("Priority_name"), Projects.Name.label(
        "Project_name")).join(Priorities, Priorities.Id == Tasks.PriorityID).join(Projects, Projects.Id == Tasks.ProjectID).where(Tasks.Recorder == session["user_id"]).all()
...
def subtasks(Id):
    """ Add subtasks """
    form = SubtaskAddForm()
    u_subtasks = dbsession.query(Subtasks.Id, Subtasks.Name, Subtasks.Description, Subtasks.SD, Subtasks.DD, Subtasks.PriorityID, Priorities.Name.label("Priority_name"),
                                 Tasks.Id.label('Task_id'), Tasks.Name.label("Task_name"), Projects.Name.label("Project_name")
                                 ).join(Priorities, Priorities.Id == Subtasks.PriorityID).join(Tasks, Tasks.Id == Subtasks.TaskID).join(Projects, Projects.Id == Tasks.ProjectID).where(Subtasks.Recorder == session["user_id"]).all()
